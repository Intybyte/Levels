...
public class GuiTypeSelection extends Screen {
    ...
    private void drawButtonTooltip(Button button, PoseStack poseStack, int mouseX, int mouseY) {
        ItemStack stack = getItemStack();
        if (stack == null) return;
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(poseStack, this.font, I18n.get("levels.misc.attributes"), width / 2, 20, 0xFFFFFF);
        drawCenteredString(poseStack, this.font, I18n.get("levels.misc.attributes.tokens") + ": " + exp.getAttributeTokens(), width / 2 - 112, 40, 0xFFFFFF);
        drawCenteredString(poseStack, this.font, I18n.get("levels.misc.attributes.current"), width / 2 + 112, 40, 0xFFFFFF);
        ...
        for (AttributeBase attribute : attributes) {
            if (attribute.hasAttribute(nbt)) {
                k++;
                drawCenteredString(poseStack, this.font, attribute.getName(nbt), width / 2 + 112, 60 + (10 * k), attribute.getHexColor());
            }
        }
